...
- 基本方針は**VSCodeの自動インポートに従う**。
- その結果として：
  - 近いファイル（同じフォルダや1〜2階層上くらい）は**相対パス** になる。
  - 遠いファイルは**エイリアス`@/`で`src/`を参照する**。
  - 同一モジュール（同じ機能やフォルダ内で完結する単位）に属するファイルは、意味的に近いため**相対パスを優先してよい**。
...
- propsの`interface`や`type`はエクスポートしない。他の場所で型が必要な場合は`ComponentProps<typeof SomeComponent>`を使用する。
